...
void sortTaskFile(char* taskfile){
    ...
    extra_file = fopen("extra.txt", "r");
    while(!feof (extra_file) && !ferror (extra_file) && getline(&buffer,&buf_size,extra_file)!=EOF){
        fprintf(main_file,"%s",buffer);
        fflush(main_file);   
    }
    fclose(extra_file);
    fclose(main_file);
